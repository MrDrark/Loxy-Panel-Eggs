#!/bin/bash
#        ====================================================
#                 SA-MP Script Egg com WINE - Criado por Drylian
#        ====================================================
# Icones 🔴 🟠 🟡 🟢 🔵 🟣 🟤 ⚫ ⚪ ✅ ❌ 📍 ✂️ 🗑️ 🟧 🟨 ⬜ 
# Icones ☑️ ✖️ ❎ 💾 📓 📗 📘 📙 📝 📖 📚 📰 🗞️ 🏷️ 🟥 🟩 🟦 ⚙️ 
# Icones 📒 📔 📕 📑 📂 📁 🗂️ 🗃️ 🗄️ 📊 📈 📉 📇 📌 🟪 🟫 ⬛    
...
StartAMD="env WINEDEBUG=-all wine64 ./samp-server.exe" # Comando Start para amd com WINE.
StartARM="env WINEDEBUG=-all wine ./samp-server.exe" # Comando Start para arm com WINE.
...
Egg="SAMP WINE          " # O Nome do egg que será executado, lembrando que o numero de caracteres maximos dentro "" é 20 oque não tiver de nome, use em espaços.
# Caminho para arquivos locais (se disponíveis)
LOCAL_PLUGINS_DIR="/home/container/.local-plugins" # Pasta para plugins locais (opcional)
    ...
    *-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*°-+*-+*-+*+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+*-+.*
    ...
    if [ "${AUTO_UPDATE}" == "1" ]; then
        ...
        if [ "$version" == "PRÉ" ]; then
            ...
            if [ "${SAMP_VOIP}" == "1" ]; then sed -i '/🟢SAMP Voip Instalado/d' ./📂Informações/🟢Informações.txt; fi
    ...
    # Adicionando Debug no Script (WINE usa .dll, não .so)
    if grep -q "crashdetect.dll" server.cfg; then
        echo "crashdetect.dll already present in plugins"
    else
        sed -i '/plugins/ s/$/ crashdetect.dll/' server.cfg
        echo "crashdetect.dll added to plugins"
    fi
    ...
    # Samp Verificador (Windows version)
    if grep -q "🟢SAMP Instalado" "./📂Informações/🟢Informações.txt"; then
        ...
        if [[ -f "./samp-server.exe" ]]; then
            ...
            echo " 🔵   Baixando SA-MP Server Windows 0.3.7 R2-1-1..."
            curl -s -L -o /tmp/samp.zip "https://github.com/drysius/Eggs/raw/main/Connect/SAMP/server/samp037_svr_R2-1-1_win32.zip"
            if [ -f /tmp/samp.zip ] && [ -s /tmp/samp.zip ]; then
                unzip -q -o /tmp/samp.zip -d /tmp/samp_temp
                # Verificar qual estrutura de pasta foi extraída
                if [ -d "/tmp/samp_temp/samp037_svr_R2-1-1_win32" ]; then
                    cp /tmp/samp_temp/samp037_svr_R2-1-1_win32/* /home/container/ -r
                elif [ -f "/tmp/samp_temp/samp-server.exe" ]; then
                    cp /tmp/samp_temp/* /home/container/ -r
                fi
                rm -rf /tmp/samp.zip /tmp/samp_temp
                echo " 🔵   O Arquivo ${C1}SA-MP${C0} foi ${C2}baixado${C0}, Continuando iniciação..."
            else
                echo " 🔴   ${C3}Falha ao baixar SA-MP. Por favor, verifique sua conexão ou baixe manualmente.${C0}"
            fi
        fi
    else
    ...
    echo " "
    # Samp-npc Verificador (Windows version)
    if grep -q "🟢SAMP Npc Instalado" "./📂Informações/🟢Informações.txt"; then
        ...
        if [[ -f "./samp-npc.exe" ]]; then
            ...
            curl -s -L -o /tmp/samp-npc.zip "https://github.com/Open-GTO/samp-npc/releases/latest/download/samp-npc-win32.zip"
            unzip -q -o /tmp/samp-npc.zip -d /tmp/samp-npc_temp
            cp /tmp/samp-npc_temp/samp-npc.exe /home/container/
            rm -rf /tmp/samp-npc.zip /tmp/samp-npc_temp
    ...
    # Samp Announce Verificador (Windows version)
    if grep -q "🟢SAMP Announce Instalado" "./📂Informações/🟢Informações.txt"; then
        ...
        if [[ -f "./announce.exe" ]]; then
            ...
            curl -s -L -o /home/container/announce.exe "https://github.com/drylian/Eggs/releases/latest/download/announce.exe"
    ...
    # Samp Voice Verificador (Windows version - .dll)
    if [ "${SAMP_VOIP}" == "1" ]; then
        echo " "
        ...
        # Verificar se existe um arquivo local para copiar (da pasta .local-plugins)
        if [[ -f "${LOCAL_PLUGINS_DIR}/sampvoice.dll" ]]; then
            echo " 🔵   Arquivo ${C1}sampvoice.dll${C0} local encontrado, copiando para plugins..."
            cp "${LOCAL_PLUGINS_DIR}/sampvoice.dll" "./plugins/sampvoice.dll"
            echo " 🔵   Arquivo ${C1}sampvoice.dll${C0} copiado com sucesso!"
        fi
        if [[ -d "plugins" ]]; then
        echo " 🔵   A pasta ${C1}Plugins${C0} foi detectada, Configurando..."
            if grep -q "🟢SAMP Voip Instalado" "./📂Informações/🟢Informações.txt"; then
                ...
                if [[ -f "./plugins/sampvoice.dll" ]]; then
                    echo " 🔵   O Arquivo ${C1}SA-MP Voip${C0} foi verificado (arquivo local encontrado), Continuando iniciação..."
                else
                    echo " 🟡   O Arquivo ${C1}SA-MP Voip${C0} ${C3}não${C0} foi encontrado, Tentando baixar..."
                    echo " 🔵   ${C2}NOTA:${C0} Se você tem um arquivo sampvoice.dll, coloque-o manualmente em ./plugins/sampvoice.dll"
                    if curl -s -f -L -o /home/container/plugins/sampvoice.dll "https://github.com/drylian/Eggs/releases/latest/download/sampvoice.dll"; then
                        echo " 🔵   O Arquivo ${C1}SA-MP Voip${C0} foi ${C2}baixado${C0}, Continuando iniciação..."
                    else
                        echo " 🔴   ${C3}Falha ao baixar sampvoice.dll.${C0} Por favor, faça upload manual do arquivo sampvoice.dll (versão Windows) em ./plugins/"
                    fi
                fi
            else
                echo " 🔵   O ${C1}SA-MP Voip${C0} ${C3}não${C0} foi detectado como Instalado, Verificando Arquivo..."
                if [[ -f "./plugins/sampvoice.dll" ]]; then
                    echo " 🟢   Arquivo ${C1}SA-MP Voip${C0} local encontrado, utilizando arquivo existente..."
                    ...
                    if curl -s -f -L -o /home/container/plugins/sampvoice.dll "https://github.com/drylian/Eggs/releases/latest/download/sampvoice.dll"; then
                        ...
                        echo " 🔴   ${C3}Falha ao baixar sampvoice.dll.${C0} Por favor, faça upload manual do arquivo sampvoice.dll (versão Windows) em ./plugins/"
                        echo " 🔴   ${C3}IMPORTANTE:${C0} Use a versão Windows (.dll), não a versão Linux (.so)"
                    fi
                fi
            fi
        else
            echo " 🟡   A Pasta ${C3}Plugins${C0} não foi encontrada, criando pasta..."
            mkdir -p ./plugins
            echo " 🔵   Verificando arquivo sampvoice.dll novamente..."
            if [[ -f "./plugins/sampvoice.dll" ]]; then
                echo " 🟢   Arquivo ${C1}SA-MP Voip${C0} local encontrado, utilizando arquivo existente..."
                echo "🟢SAMP Voip Instalado" >> "./📂Informações/🟢Informações.txt"
            else
                echo " 🟡   Tentando baixar sampvoice.dll..."
                if curl -s -f -L -o /home/container/plugins/sampvoice.dll "https://github.com/drylian/Eggs/releases/latest/download/sampvoice.dll"; then
                    echo " 🔵   O Arquivo ${C1}SA-MP Voip${C0} foi ${C2}baixado${C0}, Continuando iniciação..."
                    echo "🟢SAMP Voip Instalado" >> "./📂Informações/🟢Informações.txt"
                else
    ...
    # Server.cfg Editor (WINE - mantém .dll, não converte para .so)
    if [[ -f "./server.cfg" ]]; then
        ...
        awk '/plugins/{gsub(/.so/,"",$0);print} !/plugins/' server.cfg > server.cfg.etp.1.txt
        echo " 🔵   Configurando ${C1}Linha Plugins${C0} da ${C1}Server.cfg${C0}, Adicionando ${C2}.dll${C0} na linha plugins..."
        awk '/plugins/{for(i=2;i<=NF;i++) if($i !~ /\.dll$/) $i=$i".dll"; print} !/plugins/' server.cfg.etp.1.txt > server.cfg.etp.2.txt
        ...
        mv ./server.cfg.etp.2.txt ./server.cfg
...
|                      ${C5}INFORMAÇÕES DO SERVIDOR${C0}                      |"
    ...
    if [ "${StartType}" == "1" ]; then
        echo " 🔵   Executando em modo ${C1}Nohup(Script 2.0)${C0} com ${C1}WINE${C0}..."
        ...
        while true; do
            tail -n 10 -F server_log.txt 2>/dev/null
            tail -n 10 -F samp.erro.log.txt 2>/dev/null
            tail -n 10 -F samp.log.txt 2>/dev/null
            sleep 1
            ...
            if [ -z "$pid" ] || ! kill -0 $pid 2>/dev/null; then
                ...
                cat server_log.txt >> ./📂Informações/Logs/Server.log.txt 2>/dev/null

                if [ ! -f "./📂Informações/Logs/Samp.log.txt" ]; then
                    echo " " > "./📂Informações/Logs/Samp.log.txt"
                fi
                cat samp.log.txt >> ./📂Informações/Logs/Samp.log.txt 2>/dev/null
                ...
                cat server_log.txt >> ./📂Informações/Logs/Server.log.txt 2>/dev/null
                cat samp.log.txt >> ./📂Informações/Logs/Samp.log.txt 2>/dev/null
                cat samp.erro.log.txt >> ./📂Informações/Logs/Samp.erro.log.txt 2>/dev/null
                if [ -f "./svlog.txt" ]; then
                  	cat svlog.txt >> ./📂Informações/Logs/Voip.log.txt 2>/dev/null
                fi
        ...
        echo " 🔵   Executando em modo ${C1}Direto(Script 1.0)${C0} com ${C1}WINE${C0}..."
